...
class CFG:
    ...
    stage1_topk_candidate_dir: str = "/kaggle/working/nemotron_advanced/stage1_topk_candidates"
    stage1_topk_manifest_path: str = "/kaggle/working/nemotron_advanced/stage1_topk_manifest.csv"

    stage2_topk_candidate_dir: str = "/kaggle/working/nemotron_advanced/stage2_topk_candidates"
    stage2_topk_manifest_path: str = "/kaggle/working/nemotron_advanced/stage2_topk_manifest.csv"
...
Path(cfg.stage1_topk_candidate_dir).mkdir(parents=True, exist_ok=True)
Path(cfg.stage2_topk_candidate_dir).mkdir(parents=True, exist_ok=True)
...
train_df["source_loss_weight"] = 1.0
...
def load_optional_external_data(extra_root: str) -> pd.DataFrame:
    ...
    for path in candidates:
        ...
        if {"prompt", "answer"}.issubset(tmp.columns):
            tmp = tmp.copy()
            tmp["id"] = tmp.get("id", [f"extra_{path.stem}_{i}" for i in range(len(tmp))])
            tmp["source"] = path.stem

            # 外部数据默认降权，避免把官方分布冲掉
            if "source_loss_weight" not in tmp.columns:
                tmp["source_loss_weight"] = 0.65

            # 可选：如果外部数据本身没有 confidence，就给一个中性偏保守值
            if "confidence" not in tmp.columns:
                tmp["confidence"] = 0.95

            frames.append(tmp[["id", "prompt", "answer", "source", "source_loss_weight", "confidence"]])

    if not frames:
        return pd.DataFrame(columns=["id", "prompt", "answer", "source", "source_loss_weight", "confidence"])
...
FAMILY_TEMPLATE_ROUTER: Dict[str, List[str]] = {
    ...
    "cipher_decrypt": ["T5_text_specialized", "T1_ultra_compact", "T3_hidden_reasoning"],
    ...
    "sequence": ["T4_numeric_specialized", "T1_ultra_compact", "T3_hidden_reasoning"],
...
def choose_template_id(answer_type: str, prompt_family: str) -> str:
    # 先按题目输出形态路由，避免 open_template 被 family prior 过早锁死到 T1
    if answer_type in {"numeric", "binary"}:
        return "T4_numeric_specialized"

    # open_template 的文本题优先走 text 模板，而不是 T1
    if prompt_family == "open_template" and answer_type in {"short_text", "multi_token_text"}:
...
def build_training_record(row: pd.Series) -> Dict[str, Any]:
    ...
    return {
        ...
        "source_loss_weight": float(
            1.0 if pd.isna(getattr(row, "source_loss_weight", 1.0)) else getattr(row, "source_loss_weight", 1.0)),
        "pseudo_confidence": float(
            1.0 if pd.isna(getattr(row, "confidence", 1.0)) else getattr(row, "confidence", 1.0)),
...
def split_stage_records(records_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, set]:
    ...
    stage1_primary_mask = (
            ...
            & (
                    ...
                    |
                    (
                            records_df["answer_type"].eq("multi_token_text")
                            & records_df["prompt_family"].isin(["cipher_decrypt", "open_template"])
                    )
            )
    )
...
def compute_sample_weights(
    ...
    for row in df_like.itertuples(index=False):
        ...
        if row.prompt_family == "cipher_decrypt":
            weight *= cfg.cipher_decrypt_family_weight_boost
...
class LocalAccuracyCallback(TrainerCallback):
    ...
    def _persist_topk_manifest(self) -> None:
        ...
        out.to_csv(self.manifest_path, index=False)
...
def export_model_as_round_submission(
    model: torch.nn.Module,
    round_idx: int,
) -> Path:
    round_idx = int(round_idx)
...
def export_candidate_dir_as_round_submission(
    candidate_dir: str,
    round_idx: int,
) -> Path:
    round_idx = int(round_idx)
    ...
    if tmp_adapter_dir.exists():
        shutil.rmtree(tmp_adapter_dir, ignore_errors=True)
    shutil.copytree(candidate_dir, tmp_adapter_dir)
    ...
    print(f"[round-export] saved best-candidate round {round_idx} zip -> {zip_path}")
    print(f"[round-export] packed files -> {sorted(found)}")
    print(f"[round-export] zip size (MB) -> {round(zip_path.stat().st_size / 1024 / 1024, 3)}")
    return zip_path


def reload_saved_adapter_into_current_model(
    model: torch.nn.Module,
    adapter_dir: str,
) -> torch.nn.Module:
    target_device = "cuda:0" if torch.cuda.is_available() else "cpu"
    peft_state_dict = load_peft_weights(adapter_dir, device=target_device)
    set_peft_model_state_dict(model, peft_state_dict, adapter_name="default")
    force_nemotron_torch_fallback(model, tag=f"reload::{Path(adapter_dir).name}")
    model.config.use_cache = False
    model.train()
    return model
# %% [markdown]
...
def build_training_args(stage_name: str, lr: float, epochs: float) -> TrainingArguments:
    ...
    kwargs = dict(
        ...
        warmup_ratio=cfg.warmup_ratio if cfg.warmup_steps_override is None else 0.0,
...
def train_stage2_with_reweight(
    ...
    for round_idx in range(cfg.stage2_rounds):
        ...
        if cfg.save_stage2_round_submissions:
            ...
            if stage2_best_candidate is not None:
                export_candidate_dir_as_round_submission(
                    candidate_dir=stage2_best_candidate["candidate_dir"],
                    round_idx=round_idx + 1,
                )
            else:
                export_model_as_round_submission(
                    model=model,
                    round_idx=round_idx + 1,
                )
        ...
        probe_acc, replay_probe_pred_df = evaluate_accuracy(model, train_replay_probe_df, extractor_fn=metric_extract_prediction)
...
if not cfg.smoke_test_mode:
    managed_dirs = [
        Path(cfg.stage1_topk_candidate_dir),
        Path(cfg.stage2_topk_candidate_dir),
    ]
    managed_files = [
        Path(cfg.stage1_topk_manifest_path),
        Path(cfg.stage2_topk_manifest_path),
        Path(cfg.final_rerank_report_path),
        Path(cfg.final_export_info_path),
    ]
    ...
    for p in round_root.glob("*.zip"):
        p.unlink()

    for p in round_root.glob("round_*_adapter"):
        if p.is_dir():
...
stage1_local_callback = LocalAccuracyCallback(
    ...
    candidates_dir=None if cfg.smoke_test_mode else cfg.stage1_topk_candidate_dir,
...
stage1_best_candidate = None if cfg.smoke_test_mode else pick_best_candidate_record(cfg.stage1_topk_manifest_path)

if stage1_best_candidate is not None:
    print(
        "[stage1 best] "
        f"step={int(stage1_best_candidate['step'])} "
        f"acc={float(stage1_best_candidate['local_accuracy']):.6f} "
        f"dir={stage1_best_candidate['candidate_dir']}"
    )

    if cfg.save_stage1_round_submission:
        export_candidate_dir_as_round_submission(
            candidate_dir=stage1_best_candidate["candidate_dir"],
            round_idx=0,
        )
...
stage2_local_callback = LocalAccuracyCallback(
    ...
    extractor_fn=metric_extract_prediction,
    save_top_k=cfg.topk_candidate_keep,
    candidates_dir=None if cfg.smoke_test_mode else cfg.stage2_topk_candidate_dir,
    tokenizer=None if cfg.smoke_test_mode else tokenizer,
...
if cfg.smoke_test_mode and cfg.smoke_profile == "fast":
    ...
    model, stage2_last_pred_df, replay_buffer = train_stage2_with_reweight(
        ...
        round_callback=stage2_local_callback,
    )
